...
resource "aws_subnet" "devopsola_subnet" {
  ...
  tags = {
    Name = "devopsola-subnet-${count.index}"
  }
}
...
resource "aws_security_group" "devopsola_node_sg" {
  vpc_id = aws_vpc.devopsola_vpc.id
...
resource "aws_eks_node_group" "devopsola" {
  ...
  remote_access {
    ...
    source_security_group_ids = [aws_security_group.devopsola_node_sg.id]
  }
}
